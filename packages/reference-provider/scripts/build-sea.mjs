...
const providerName = getArg("--name") ?? "dummycloud";
const outDir       = path.resolve(providerRoot, getArg("--out") ?? "bin-sea");
...
//
// esbuild marks node: builtins and .node native addons as external
// automatically. This produces a single self-contained CJS file that
// Node SEA can embed.
// ---------------------------------------------------------------------------
...
await esbuild.build({
  ...
  format: "cjs",
  outfile: bundleOut,
  // Keep node: builtins and .node native addons external
  packages: "bundle",
  external: ["*.node"],
});
...
console.log(`\n✅  SEA binary ready: ${binaryPath}  (${sizeMb} MB)`);
console.log(`\n   Smoke test:`);
