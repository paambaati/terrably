...
describe("GetFunctions", () => {
  it("returns an empty map when provider has no functions", async () => {
  ...
  it("returns a schema entry for each registered function", async () => {
  ...
  it("populates parameter name and type for a single-parameter function", async () => {
  ...
  it("populates summary", async () => {
    const svc = new ProviderServicer(makeProvider([UppercaseFunction]));
    const res = await svc.GetFunctions({} as never, {});
    assert.equal(res.functions!["uppercase"]!.summary, "Convert a string to uppercase");
  });

  it("populates variadic_parameter when present", async () => {
  ...
  it("returns undefined variadic_parameter for non-variadic functions", async () => {
    const svc = new ProviderServicer(makeProvider([UppercaseFunction]));
    const res = await svc.GetFunctions({} as never, {});
    assert.equal(res.functions!["uppercase"]!.variadicParameter, undefined);
  });

  it("includes function schemas in GetProviderSchema.functions", async () => {
    const svc = new ProviderServicer(makeProvider([UppercaseFunction]));
...
describe("CallFunction — basic dispatch", () => {
  it("calls uppercase and returns uppercased string", async () => {
    const svc = new ProviderServicer(makeProvider([UppercaseFunction]));
  ...
  it("calls add and returns the sum", async () => {
...
describe("CallFunction — variadic", () => {
  it("joins values with the separator", async () => {
    const svc = new ProviderServicer(makeProvider([ConcatFunction]));
    const res = await svc.CallFunction(
  ...
  it("works with zero variadic args (only required parameter provided)", async () => {
    const svc = new ProviderServicer(makeProvider([ConcatFunction]));
    const res = await svc.CallFunction(
...
describe("CallFunction — arity errors", () => {
  it("returns error when too few arguments are passed", async () => {
    const svc = new ProviderServicer(makeProvider([AddFunction]));
  ...
  it("returns error when too many arguments are passed (non-variadic)", async () => {
    const svc = new ProviderServicer(makeProvider([UppercaseFunction]));
...
describe("CallFunction — unknown function", () => {
  it("returns FunctionError when function name is not registered", async () => {
    const svc = new ProviderServicer(makeProvider([UppercaseFunction]));
...
describe("CallFunction — diagnostic error in call()", () => {
  it("converts a diagnostic error to FunctionError", async () => {
...
describe("CallFunction — exception thrown in call()", () => {
  it("converts a thrown exception to FunctionError", async () => {
