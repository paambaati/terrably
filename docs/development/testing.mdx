...
| `provider: compiled binary` | standalone binary | `bin/` |

<Callout type="warn">
The compiled-binary suite requires the provider binary to be built first (`pnpm run build`). It also requires `terraform` to be on your `PATH`.
</Callout>
